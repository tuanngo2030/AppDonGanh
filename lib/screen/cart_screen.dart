...
Future<void> removeItem(String gioHangId, String variantId) async {
  try {
    ...
    for (var cart in cartData) {
      for (var sanPhamCart in cart.mergedCart) {
        for (var sanPhamList in sanPhamCart.sanPhamList) {
          sanPhamList.chiTietGioHangs.removeWhere(
            (chiTiet) => chiTiet.variantModel.id == variantId,
          );
        }
    ...
    final updatedCartData = cartData.where((cart) => cart.mergedCart.isNotEmpty).toList();

    // Update the total price
    double updatedTotalPrice = 0;
    for (var variant in selectedVariants) {
      updatedTotalPrice += variant.donGia * variant.soLuong;
    }

    // Update the cart data and total price
    setState(() {
                      ...
                      children: [
                        ...
                        ...sanPhamCart.sanPhamList.map((sanPhamItem) {
                          return Card(
                            ...
                            child: Column(
                              children: sanPhamItem.chiTietGioHangs.map((chiTiet) {
                                groupTotal += chiTiet.donGia * chiTiet.soLuong;
                                ...
                                return Padding(
                                  ...
                                  child: Row(
                                    ...
                                    children: [
                                      Checkbox(
                                        value: selectedVariants.contains(chiTiet),
                                                    ...
                                                    IconButton(
                                                      ...
                                                      onPressed: () {
                                                        if (chiTiet.soLuong > 1) {
                                                          ...
                                                          updateItem(cart.id, chiTiet.variantModel.id, chiTiet.soLuong, chiTiet.donGia, chiTiet.id);
                                                        }
                                                      },
                                              ...
                                              return AlertDialog(
                                                ...
                                                actions: <Widget>[
                                                  ...
                                                  TextButton(
                                                    ...
                                                    onPressed: () {
                                                      Navigator.of(context).pop(); // Close dialog
                                                      removeItem(cart.id, chiTiet.variantModel.id); // Perform delete
                                                    },
                                                  ),
                                                ],
                                              );
                    ...
                    ElevatedButton(
                      ...
                      onPressed: selectedVariants.isNotEmpty
                          ? () {
                              ...
                              final selectedCart = cartList.map((cart) {
                                final filteredMergedCart = cart.mergedCart.map((sanPhamCart) {
                                  final filteredSanPhamList = sanPhamCart.sanPhamList.map((sanPhamItem) {
                                    final filteredChiTiet = sanPhamItem.chiTietGioHangs.where((chiTiet) => selectedVariants.contains(chiTiet)).toList();
                                    return filteredChiTiet.isNotEmpty
                                        ? SanPhamList(user: sanPhamItem.user, sanPham: sanPhamItem.sanPham, chiTietGioHangs: filteredChiTiet)
                                        : null;
                                  }).whereType<SanPhamList>().toList();
                                  return filteredSanPhamList.isNotEmpty ? SanPhamCart(user: sanPhamCart.user, sanPhamList: filteredSanPhamList) : null;
                                }).whereType<SanPhamCart>().toList();
                                return filteredMergedCart.isNotEmpty ? CartModel(id: cart.id, user: cart.user, mergedCart: filteredMergedCart) : null;
